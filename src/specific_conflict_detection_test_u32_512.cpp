  ...
  template<typename T, size_t VectorSizeInBit>
    ...
    double conflict_detection_fpga_wrapper(
      ...
      q->submit([&](handler& h) {
          h.single_task([=]() [[intel::kernel_args_restrict]] {
              ...
              device_ptr<uint32_t> out(out_ptr);
#else
    ...
    void bench_and_test_vector_size(
      ...
      file_logging::log_to_file<true>(
        ...
        "uint32_t", 
        std::to_string(512), 
        std::to_string(element_count*sizeof(uint32_t)), 
        std::to_string(offloading_and_execution_time), 
        std::to_string(execution_time), 
        std::to_string((((element_count * sizeof(uint32_t)) / (1024.0 * 1024.0)) / (execution_time / 1000000.0))),
        file_logging::concat_pointer_to_string<uint32_t>(data, 10),
        file_logging::concat_pointer_to_string<uint32_t>(result_buffer_fpga_on_host, 10),
        "-", "-"
      );
    ...
    void bench_and_test_data_type(
      ...
      auto in = create_buffer<uint32_t>(q, MALLOC_TARGET::HOST, data_size, ALIGNMENT);
#ifdef FPGA_WRITE_TO_DEV
      auto out_fpga = create_buffer<uint32_t>(q, MALLOC_TARGET::DEVICE, data_size, ALIGNMENT);
#else
      auto out_fpga = create_buffer<uint32_t>(q, MALLOC_TARGET::HOST, data_size, ALIGNMENT);
#endif
      auto out_host = create_buffer<uint32_t>(q, MALLOC_TARGET::HOST, data_size, ALIGNMENT);
