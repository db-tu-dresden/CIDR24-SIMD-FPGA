...
namespace tuddbs{
  ...
  template<typename T, size_t VectorSizeInBit>
    void conflict_detection_scalar(
      ...
      std::array<uint64_t, VectorSizeInBit/(CHAR_BIT*sizeof(T))> results{};
    ...
    auto conflict_detection_fpga_wrapper(
      ...
      q->submit([&](handler& h) {
          h.single_task([=]() [[intel::kernel_args_restrict]] {
              ...
              [[intel::fpga_register]] std::array<uint64_t, 8> data_vec{};
              [[intel::fpga_register]] std::array<uint64_t, 8> result_vec{};
              [[intel::fpga_register]] std::array<uint64_t, 8> conflict_vec{};
              auto const data_end = in + element_count;
              for (; in != data_end; in+=8) {
                #pragma unroll
    ...
    void bench_and_test_vector_size(
      ...
      conflict_detection_scalar<uint64_t, 512>(out_host, data, element_count);
      ...
      for (size_t i = 0; i < element_count; ++i) {
        if (out_host[i] != out_fpga[i]) {
          ...
          auto const batch_size = 512/(CHAR_BIT*sizeof(uint64_t));
      ...
      writer.write_line(
        ...
        "conflict_detection",
        "unrolled",
        tsl::type_name<uint64_t>(), 512,
        element_count,
        element_count * sizeof(uint64_t),
        default_clock_t::duration(start_offload_exec, end_offload_exec), default_clock_t::duration(start_exec, end_exec),
        default_tput_t::throughput(start_exec, end_exec, element_count * sizeof(uint64_t)),
        file_logging::concat_pointer_to_string<uint64_t>(out_host, 10), file_logging::concat_pointer_to_string<uint64_t>(out_fpga, 10)
      );
    }
    ...
    void bench_and_test_data_type(
      ...
      auto in = create_buffer<uint64_t>(q, MALLOC_TARGET::HOST, data_size, 64);
      auto out_fpga = create_buffer<uint64_t>(q, MALLOC_TARGET::HOST, data_size, 64);
      auto out_host = create_buffer<uint64_t>(q, MALLOC_TARGET::HOST, data_size, 64);
