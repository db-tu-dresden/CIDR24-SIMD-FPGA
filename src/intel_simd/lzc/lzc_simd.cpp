...
namespace tuddbs{
  ...
  template<typename T>
    void run_lzc_scalar(
      ...
      for (; data != data_end; ++data, ++out) {
        *out = std::countl_zero<std::make_unsigned_t<T>>(*data);
      }
    }
    ...
    void run_lzc_sse(
      ...
      for (; data < data_end; data += SimdStepWidth, out += SimdStepWidth) {
        ...
        if constexpr (sizeof(T) == 4) {
          result_vec = _mm_lzcnt_epi32(data_vec);
        } else {
          result_vec = _mm_lzcnt_epi64(data_vec);
        }
        if constexpr (MTK == MEM_TRANSFER_KIND::LOAD) {
  ...
  template<typename T, MEM_TRANSFER_KIND MTK, size_t SimdStepWidth = (32/sizeof(T))>
    void run_lzc_avx(
      ...
      for (; data < data_end; data += SimdStepWidth, out += SimdStepWidth) {
        ...
        if constexpr (sizeof(T) == 4) {
          result_vec = _mm256_lzcnt_epi32(data_vec);
        } else {
          result_vec = _mm256_lzcnt_epi64(data_vec);
        }
        if constexpr (MTK == MEM_TRANSFER_KIND::LOAD) {
  ...
  template<typename T, MEM_TRANSFER_KIND MTK, size_t SimdStepWidth = (64/sizeof(T))>
    void run_lzc_avx512(
      ...
      for (; data < data_end; data += SimdStepWidth, out += SimdStepWidth) {
        ...
        if constexpr (sizeof(T) == 4) {
          result_vec = _mm512_lzcnt_epi32(data_vec);
        } else {
          result_vec = _mm512_lzcnt_epi64(data_vec);
        }
        if constexpr (MTK == MEM_TRANSFER_KIND::LOAD) {
  ...
  template<typename T>
    void run_benchmark(
      ...
      // validate
      run_lzc_scalar<T>(out_reference, data, element_count);
      run_lzc_sse<T, MEM_TRANSFER_KIND::LOAD>(out, data, element_count);
      for (size_t i = 0; i < element_count; ++i) {
        if (out_reference[i] != out[i]) {
          ...
          std::cerr << std::endl;
          throw std::runtime_error("Error in lzc.");
        }
      }
      run_lzc_avx<T, MEM_TRANSFER_KIND::LOAD>(out, data, element_count);
      for (size_t i = 0; i < element_count; ++i) {
        if (out_reference[i] != out[i]) {
      ...
      run_lzc_avx512<T, MEM_TRANSFER_KIND::LOAD>(out, data, element_count);
      for (size_t i = 0; i < element_count; ++i) {
        if (out_reference[i] != out[i]) {
      ...
      { // SSE Cached Load
        for (size_t i = 0; i < REPEATS; ++i) {
          run_lzc_sse<T, MEM_TRANSFER_KIND::LOAD>(flush_out, flush_data, element_count);
      ...
      { // SSE Cached Load
        for (size_t i = 0; i < REPEATS; ++i) {
          run_lzc_sse<T, MEM_TRANSFER_KIND::STREAM>(flush_out, flush_data, element_count);
          auto start = tuddbs::current_time();
          run_lzc_sse<T, MEM_TRANSFER_KIND::STREAM>(out, data, element_count);
          auto end = tuddbs::current_time();
          auto execution_time = tuddbs::time_diff(start, end);
          file_logging::log_to_file<true>(
            cpu_name,
      ...
      { // AVX Cached Load
        for (size_t i = 0; i < REPEATS; ++i) {
          run_lzc_avx<T, MEM_TRANSFER_KIND::LOAD>(flush_out, flush_data, element_count);
          auto start = tuddbs::current_time();
          run_lzc_avx<T, MEM_TRANSFER_KIND::LOAD>(out, data, element_count);
          auto end = tuddbs::current_time();
          auto execution_time = tuddbs::time_diff(start, end);
      ...
      { // AVX Stream Load
        for (size_t i = 0; i < REPEATS; ++i) {
          run_lzc_avx<T, MEM_TRANSFER_KIND::STREAM>(flush_out, flush_data, element_count);
          auto start = tuddbs::current_time();
          run_lzc_avx<T, MEM_TRANSFER_KIND::STREAM>(out, data, element_count);
          auto end = tuddbs::current_time();
          auto execution_time = tuddbs::time_diff(start, end);
          file_logging::log_to_file<true>(
            cpu_name,
      ...
      { // AVX512 Cached Load
        for (size_t i = 0; i < REPEATS; ++i) {
          run_lzc_avx512<T, MEM_TRANSFER_KIND::LOAD>(flush_out, flush_data, element_count);
          auto start = tuddbs::current_time();
          run_lzc_avx512<T, MEM_TRANSFER_KIND::LOAD>(out, data, element_count);
          auto end = tuddbs::current_time();
          auto execution_time = tuddbs::time_diff(start, end);
      ...
      { // AVX Cached Load      
        for (size_t i = 0; i < REPEATS; ++i) {
          run_lzc_avx512<T, MEM_TRANSFER_KIND::STREAM>(flush_out, flush_data, element_count);
          auto start = tuddbs::current_time();
          run_lzc_avx512<T, MEM_TRANSFER_KIND::STREAM>(out, data, element_count);
          auto end = tuddbs::current_time();
          auto execution_time = tuddbs::time_diff(start, end);
          file_logging::log_to_file<true>(
            cpu_name,
  ...
  template <typename T>
    void bench_intel_intrin(
      ...
      std::uniform_int_distribution<T> dist_data(1, std::numeric_limits<T>::max());
