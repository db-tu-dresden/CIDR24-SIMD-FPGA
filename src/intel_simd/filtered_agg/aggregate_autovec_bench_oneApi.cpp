    ...
    void run_aggregate(
      ...
      q->submit([&](handler& h) {
        h.single_task([=]() [[intel::kernel_args_restrict]] {
          ...
          *out = aggregate_autovec_N<T, VectorSize/(sizeof(T)*CHAR_BIT), HORIZONTAL_AGGREGATION_STRATEGY::ACCUM>(in, byte_size/sizeof(T));
        });
      }).wait();
...
#ifdef VL128
    run_aggregate<T, 128>(q, reinterpret_cast<T const *>(measurement_data), byte_size, writer);
#endif
#ifdef VL256
    run_aggregate<T, 256>(q, reinterpret_cast<T const *>(measurement_data), byte_size, writer);
#endif
#ifdef VL512
    run_aggregate<T, 512>(q, reinterpret_cast<T const *>(measurement_data), byte_size, writer);
#endif
#ifdef VL1024
    run_aggregate<T, 1024>(q, reinterpret_cast<T const *>(measurement_data), byte_size, writer);
#endif
#ifdef VL2048
    run_aggregate<T, 2048>(q, reinterpret_cast<T const *>(measurement_data), byte_size, writer);
#endif
#ifdef VL4096
    run_aggregate<T, 4096>(q, reinterpret_cast<T const *>(measurement_data), byte_size, writer);
#endif
